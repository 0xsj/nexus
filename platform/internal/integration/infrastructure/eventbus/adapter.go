...
// Adapter bridges integration domain.EventPublisher to pkg/eventbus.Publisher.
type Adapter struct {
...
// NewAdapter creates a new eventbus adapter for Integration.
func NewAdapter(publisher eventbus.Publisher) *Adapter {
	return &Adapter{publisher: publisher}
}

// Publish converts integration domain events to EventEnvelopes and publishes synchronously.
