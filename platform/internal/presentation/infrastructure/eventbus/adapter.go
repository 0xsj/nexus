...
// Adapter bridges presentation domain.EventPublisher to pkg/eventbus.Publisher.
type Adapter struct {
...
// NewAdapter creates a new eventbus adapter for Presentation.
func NewAdapter(publisher eventbus.Publisher) *Adapter {
	return &Adapter{publisher: publisher}
}

// Publish converts presentation domain events to EventEnvelopes and publishes synchronously.
