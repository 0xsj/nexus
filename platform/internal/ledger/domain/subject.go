...
// SubjectValidation creates a subject validation error.
func SubjectValidation(operation string, field string, reason string) *pkgerrors.Error {
	return pkgerrors.Validation(operation, reason).
		WithCode(CodeInvalidSubjectID).
...
const (
	// SubjectTypeUnknown indicates an unknown subject type.
	// Used for forward compatibility when new bounded contexts emit events
	// before the Ledger is updated to recognize them.
	SubjectTypeUnknown SubjectType = iota
...
// IsValid returns true if the SubjectType is a known, valid type.
// Note: SubjectTypeUnknown returns false here, but is still allowed
// in NewSubject for forward compatibility.
func (t SubjectType) IsValid() bool {
	switch t {
	case SubjectTypeUser, SubjectTypeCredential, SubjectTypePresentation,
		SubjectTypeVerification, SubjectTypeWallet, SubjectTypeSession,
...
// IsKnown returns true if the SubjectType is a recognized type (including Unknown).
func (t SubjectType) IsKnown() bool {
	return t >= SubjectTypeUnknown && t <= SubjectTypeVouch
}
...
// NewSubject creates a new Subject.
// Allows SubjectTypeUnknown for forward compatibility in audit logging.
// This ensures events from new bounded contexts are never lost.
func NewSubject(subjectType SubjectType, subjectID SubjectID) (Subject, error) {
	if !subjectType.IsKnown() {
		return Subject{}, InvalidSubjectType("domain.NewSubject", subjectType.String())
	}
	if subjectID.IsZero() {
		return Subject{}, InvalidSubjectID("domain.NewSubject", "", "subject ID cannot be empty")
