...
// Adapter bridges trust domain.EventPublisher to pkg/eventbus.Publisher.
type Adapter struct {
...
// NewAdapter creates a new eventbus adapter for Trust.
func NewAdapter(publisher eventbus.Publisher) *Adapter {
	return &Adapter{publisher: publisher}
}

// Publish converts trust domain events to EventEnvelopes and publishes synchronously.
