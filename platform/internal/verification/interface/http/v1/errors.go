...
func MapError(err error) (int, ErrorResponse) {
	...
	// Check for verification domain errors
	if pkgerrors.Is(err, domain.ErrVerificationNotFound) {
		return http.StatusNotFound, ErrorResponse{
			Code:    string(domain.CodeVerificationNotFound),
			Message: err.Error(),
		}
	}

	if pkgerrors.Is(err, domain.ErrVerificationAlreadyCompleted) {
		return http.StatusConflict, ErrorResponse{
			Code:    string(domain.CodeVerificationAlreadyCompleted),
			Message: err.Error(),
		}
	}
	...
	if pkgerrors.Is(err, domain.ErrOAuthCallbackFailed) {
		return http.StatusBadGateway, ErrorResponse{
			Code:    string(domain.CodeOAuthCallbackFailed),
			Message: err.Error(),
		}
	}
	...
	if pkgerrors.Is(err, domain.ErrVerificationFailed) {
		return http.StatusBadRequest, ErrorResponse{
			Code:    string(domain.CodeVerificationFailed),
			Message: err.Error(),
		}
	}

	if pkgerrors.Is(err, domain.ErrDataFetchFailed) {
		return http.StatusBadGateway, ErrorResponse{
			Code:    string(domain.CodeDataFetchFailed),
			Message: err.Error(),
		}
	}
