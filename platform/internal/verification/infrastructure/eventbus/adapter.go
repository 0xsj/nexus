...
// Adapter bridges verification domain.EventPublisher to pkg/eventbus.Publisher.
type Adapter struct {
...
// NewAdapter creates a new eventbus adapter for Verification.
func NewAdapter(publisher eventbus.Publisher) *Adapter {
	return &Adapter{publisher: publisher}
}

// Publish converts verification domain events to EventEnvelopes and publishes synchronously.
