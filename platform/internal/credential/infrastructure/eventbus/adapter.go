...
// Adapter bridges credential domain.EventPublisher to pkg/eventbus.Publisher.
type Adapter struct {
...
// NewAdapter creates a new eventbus adapter for Credential.
func NewAdapter(publisher eventbus.Publisher) *Adapter {
	return &Adapter{publisher: publisher}
}

// Publish converts credential domain events to EventEnvelopes and publishes synchronously.
